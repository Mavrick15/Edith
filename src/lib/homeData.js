export const featureListData = [
  {
    iconSrc: "/images/home_1/compassion.svg",
    title: "Compassion",
    subTitle:
      "Nous comprenons que consulter un médecin peut être une expérience stressante et émotionnelle. Nous nous efforçons de créer un environnement accueillant et bienveillant qui met nos patients en confiance.",
  },
  {
    iconSrc: "/images/home_1/excellence.svg",
    title: "Excellence",
    subTitle:
      "Nous nous engageons à fournir des soins médicaux et des services d'excellence à nos patients. Nous croyons en l'amélioration continue de nos compétences, connaissances et ressources pour offrir les meilleurs soins possibles.",
  },
  {
    iconSrc: "/images/home_1/integrity.svg",
    title: "Intégrité",
    subTitle:
      "Nous croyons en une pratique de la médecine avec intégrité et honnêteté. Nous sommes transparents dans notre communication et nos processus décisionnels, et nous plaçons toujours les intérêts de nos patients en premier.",
  },
  {
    iconSrc: "/images/home_1/respect.svg",
    title: "Respect",
    subTitle:
      "Nous traitons toutes les personnes avec respect et dignité, quelles que soient leurs origines, croyances ou circonstances. Nous croyons que chaque personne mérite d'être traitée avec compassion et bienveillance.",
  },
  {
    iconSrc: "/images/home_1/teamwork.svg",
    title: "Travail\u00A0d'équipe",
    subTitle:
      "Nous croyons en la collaboration avec nos collègues et les autres professionnels de santé pour fournir des soins complets et efficaces à nos patients.",
  },
];

export const brandData = [
  { imgUrl: "/images/brand_1.png", imgAlt: "Brand" },
  { imgUrl: "/images/brand_2.png", imgAlt: "Brand" },
  { imgUrl: "/images/brand_3.png", imgAlt: "Brand" },
  { imgUrl: "/images/brand_4.png", imgAlt: "Brand" },
  { imgUrl: "/images/brand_5.png", imgAlt: "Brand" },
  { imgUrl: "/images/brand_6.png", imgAlt: "Brand" },
  { imgUrl: "/images/brand_7.png", imgAlt: "Brand" },
  { imgUrl: "/images/brand_8.png", imgAlt: "Brand" },
];

export const faqData = [
  {
    title: "Quels services Edith propose-t-il ?",
    content:
      "Edith offre une gamme complète de services médicaux incluant les consultations générales, les spécialités, les examens de routine, la consultation gynécologique et bien plus encore. Contactez-nous pour plus d'informations.",
  },
  {
    title: "Comment prendre rendez-vous avec Edith ?",
    content:
      "Vous pouvez prendre rendez-vous en ligne via notre formulaire, par téléphone ou en vous rendant à l'accueil. Nos équipes sont disponibles pour vous aider à trouver le créneau qui vous convient.",
  },
  {
    title: "Acceptez-vous les assurances ?",
    content:
      "Oui, nous acceptons la plupart des assurances maladie. N'hésitez pas à nous contacter pour vérifier si votre mutuelle est partenaire et quels sont les remboursements applicables.",
  },
  {
    title: "Que dois-je apporter à mon rendez-vous ?",
    content:
      "Pensez à apporter votre carte vitale, votre attestation de mutuelle, vos ordonnances en cours et tout document médical pertinent. Pour un premier rendez-vous, remplissez notre dossier patient à l'avance.",
  },
  {
    title: "Comment demander un renouvellement d'ordonnance ?",
    content:
      "Vous pouvez demander un renouvellement via votre espace patient en ligne, par téléphone ou lors de votre prochaine consultation. Un délai de traitement peut s'appliquer selon votre situation.",
  },
];

export const awardData = [
  {
    title: "Label qualité Ministère de la Santé RDC",
    subTitle:
      "Certification délivrée par le Ministère de la Santé Publique pour les établissements de santé répondant aux normes nationales de qualité des soins et d'hygiène.",
    iconUrl: "/images/icons/award.svg",
  },
  {
    title: "Prix d'excellence Université de Kinshasa",
    subTitle:
      "Récompense décernée par la Faculté de Médecine de l'Unikin aux structures partenaires pour leur engagement dans la formation des futurs professionnels de santé.",
    iconUrl: "/images/icons/award.svg",
  },
  {
    title: "Distinction Ordre des Médecins du Congo",
    subTitle:
      "Reconnaissance de l'Ordre national des Médecins pour l'excellence des pratiques médicales et le respect de la déontologie professionnelle.",
    iconUrl: "/images/icons/award.svg",
  },
  {
    title: "Prix santé maternelle Kinshasa",
    subTitle:
      "Distinction attribuée par les autorités provinciales pour la qualité des soins en gynéco-obstétrique et la réduction de la mortalité maternelle et infantile.",
    iconUrl: "/images/icons/award.svg",
  },
];

export const departmentData = [
  {
    title: "Gynécologie obstétrique",
    iconUrl: "/images/home_1/department_icon_5.svg",
    href: "/services/gynecologie-obstetrique",
  },
  {
    title: "Consultation gynécologique",
    iconUrl: "/images/home_1/department_icon_8.svg",
    href: "/services/consultation-gynecologique",
  },
  {
    title: "Soins infertilité",
    iconUrl: "/images/home_1/department_icon_1.svg",
    href: "/services/soins-infertilite",
  },
  {
    title: "Maternité",
    iconUrl: "/images/home_1/department_icon_3.svg",
    href: "/services/maternite",
  },
  {
    title: "Pédiatrie",
    iconUrl: "/images/home_1/department_icon_3.svg",
    href: "/services/pediatrie",
  },
  {
    title: "Imagerie médicale",
    iconUrl: "/images/home_1/department_icon_9.svg",
    href: "/services/imagerie-medicale",
  },
  {
    title: "Laboratoire biomédical",
    iconUrl: "/images/home_1/department_icon_2.svg",
    href: "/services/laboratoire-biomedical",
  },
  {
    title: "Hospitalisation",
    iconUrl: "/images/home_1/department_icon_4.svg",
    href: "/services/hospitalisation",
  },
  {
    title: "Chirurgie",
    iconUrl: "/images/home_1/department_icon_6.svg",
    href: "/services/chirurgie",
  },
];
