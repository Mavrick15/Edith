  ...
  return (
    <div className="cs_doctor_profile cs_style_1">
      <div className="cs_doctor_img">
        <Image 
          src={imgUrl} 
          alt={`Photo de ${name}, ${designation}`} 
          className="cs_radius_25" 
          height={771} 
          width={727}
          sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 727px"
          loading="lazy"
        />
      </div>
      <div className="cs_doctor_profile_right">
        <div className="cs_doctor_info cs_radius_25 cs_white_bg overflow-hidden">
          <div className="cs_doctor_info_in">
            ...
            <p className="mb-0">{safeParse(description)}</p>
            <div className="cs_height_28" />
            <div className="cs_social_links" role="group" aria-label="Réseaux sociaux">
              {social?.map((item, index) => {
                // Extraire le nom du réseau social de l'icône (ex: "fa6-brands:facebook-f" -> "Facebook")
                const getSocialName = (icon) => {
                  ...
                  return socialNames[iconName] || iconName;
                };
                ...
                return (
                  <Link 
                    ...
                    aria-label={`Suivre ${name} sur ${getSocialName(item.icon)}`}
                  >
          ...
          <div className="cs_doctor_meta cs_accent_bg">
            <div className="cs_doctor_meta_left cs_white_color">
              {availableUrl && (
                <Link href={availableUrl} aria-label={`Vérifier la disponibilité de ${name}`}>
                  <Image src="/images/departments/clock.svg" alt="" height={20} width={20} aria-hidden="true" />
                  Disponibilité
                </Link>
              )}
              {callUrl && (
                <Link href={callUrl} aria-label={`Appeler ${name}`}>
                  <Image src="/images/departments/call.svg" alt="" height={20} width={20} aria-hidden="true" />
                  Appeler
                </Link>
              )}
              {chatUrl && (
                <Link href={chatUrl} aria-label={`Chatter avec ${name}`}>
                  <Image src="/images/departments/chat.svg" alt="" height={20} width={20} aria-hidden="true" />
                  Chat
                </Link>
              )}
            </div>
            <div className="cs_doctor_meta_right cs_white_color cs_semibold">
              <Link href={btnUrl} aria-label={btnText || `En savoir plus sur ${name}`}>
                <span>{btnText}</span>
                <svg
                  width={14}
                  height={10}
